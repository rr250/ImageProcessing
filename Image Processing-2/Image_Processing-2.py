...
def roi_per_csv(image, csvfile, inpath, outpath, cwd):
    try:
        ...
        for row in csv_reader:
            cord=row[2]
            filename=row[0]
            if not os.path.exists(os.path.join(outpath,csvfile)):
                os.makedirs(os.path.join(outpath,csvfile))
            if '-' in cord:
                mcords=cord.split("-")
                subfilenames=row[4].split("?")
                subfilenames.pop()
                if not os.path.exists(os.path.join(outpath,csvfile,filename)):
                    os.makedirs(os.path.join(outpath,csvfile,filename))
                i=0
                for mcord in mcords:
                    x,y,w,h = list(map(int, mcord.split(",")))
                    sub_img = image[y:y+h,x:x+w]
                    if subfilenames[0]=='':
                        cv2.imwrite(os.path.join(cwd,outpath,csvfile,filename,str(i)+'.jpg'), sub_img)
                    else:
                        cv2.imwrite(os.path.join(cwd,outpath,csvfile,filename,subfilenames[i]+'.jpg'), sub_img)
                        
                    i+=1
            else:
                x,y,w,h = list(map(int, cord.split(",")))
                sub_img = image[y:y+h,x:x+w]
                cv2.imwrite(os.path.join(cwd,outpath,csvfile,filename+'.jpg'), sub_img)
    except Exception as e:
        logging.error("Exception occurred", exc_info=True)
